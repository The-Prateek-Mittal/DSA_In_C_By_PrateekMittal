/* Singly Linked List using Doubly linked list */
#include <stdio.h>
#include <stdlib.h>
...
void LIST_DELETE(int y)
{
  ...
  struct node *yprevNode = L_head;
  if(yprevNode == yNode)
  {
    L_head = yNode->next;
  }
  while (yprevNode->next != NULL)
  { 
    if(yprevNode->next == yNode)
    {
      yprevNode->next = yNode->next;
      break;
    }
    else 
    {
      yprevNode = yprevNode->next;
    }
  }
